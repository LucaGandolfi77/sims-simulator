...
void DrawTrees(const std::vector<Tree>& trees) {
    for(const auto& tree : trees) {
        ...
        // Fruits (Visible apples ON the leaves)
        for(int i = 0; i < tree.fruitCount; i++) {
            // Distribute mostly on the upper half/sides to be visible
            float angle = (float)i * (360.0f / 5.0f);
            float rad = angle * (3.14159f / 180.0f);
            
            // Place on the surface of the sphere (Radius 2.0)
            // A bit above the equator so they sit "on" the tree
            float yOffset = 0.8f; 
            float radiusAtY = sqrtf(4.0f - yOffset*yOffset); // r^2 - y^2
            
            Vector3 fruitPos = {
                leavesPos.x + cosf(rad) * radiusAtY,
                leavesPos.y + yOffset, 
                leavesPos.z + sinf(rad) * radiusAtY
            };
            DrawSphere(fruitPos, 0.4f, RED); // Slightly larger, visible apples
        }
    }
}
...
void DrawHouses(const std::vector<House>& houses) {
    // Transparent walls but visible enough to see Sims inside (Alpha 120)
    Color wallColor = { 200, 200, 255, 120 }; 
...
void DrawOffice(const Office& off) {
    Color wallColor = { 100, 100, 100, 120 }; // Gray Transparent - Visible
